...
class EventDetector(object):
    ...
    def process_stream(self):
        for tweet in self.stream:
            ...
            self.temporalprocessor.process(time_t, id_str, terms)
...
class TemporalProcessor(object):
    ...
    def process(self, time_tuple, id_str, terms):
...
if __name__ == '__main__':
    ts = TwitterStream()
    datapath = '/Users/shengxia/Documents/data/raw_data'
    dirlist = os.listdir(datapath)
    for path in dirlist:
        if path.startswith('201') and os.path.isdir(os.path.join(datapath,path)):
            ts.source(os.path.join(datapath,path))
    ts.sort()
    temp = TemporalProcessor(3600,0.3,0.0002,8,result_dir='/Users/shengxia/Documents/data/result_0312')
